# cl3_cluster_builder_v2.py
"""
Builder for Cluster 3 (Civil Security for Society) using the *new* grouped JSONs.
...
Creates in Neo4j the same shape as CL2 builder.
"""
import argparse
import json
import re
...
class ClusterGraphBuilderCL3v2:
...
def main():
    ...
    ap.add_argument("--grouped", required=True, help="Path to cluster_CL3.grouped.json (grouped by destination)")
    ap.add_argument("--summaries", required=True, help="Path to destination_summaries_cl3.json")
    ap.add_argument("--preview", action="store_true", help="Print queries instead of executing them")
    args = ap.parse_args()

    builder = ClusterGraphBuilderCL3v2(preview=args.preview)
