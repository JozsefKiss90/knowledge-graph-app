...
/**
 * B5 — Research-field explorer: a side drawer (Compare pattern, createPortal to body) that lets you browse
 * the EuroSciVoc research-field hierarchy and, for any field, see how many EU-funded projects sit under it
 * and which Horizon Europe calls are funded in it. Clicking a call opens its detail page (/node/:id) — a
 * subject-first path into the graph, complementing the programme-first drill-down. Pure read of existing
 * CORDIS edges. Honest: counts are EU-funded participation, not quality/impact, and overlap across branches.
 */
export default function CordisFieldExplorerDrawer({ open, onClose }) {
  const { loading, data, error } = useCordisFieldTree("default", open);
  ...
  const card = (
    <Box className="cordis-fields-drawer">
      <Box className="cordis-fields-drawer__header">
        <AccountTreeIcon sx={{ fontSize: 20, color: "var(--primary)" }} />
