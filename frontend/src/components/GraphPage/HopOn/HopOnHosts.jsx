...
/**
 * B6 — Hop-on host finder (dashboard panel body): the ongoing Horizon Europe Pillar II / EIC Pathfinder
 * collaborative projects a widening-country partner could join via the Hop-on Facility, and — per host —
 * which widening countries are NOT yet in the consortium (the actual opening). Filterable by research field,
 * cluster/programme, "missing widening country", and minimum remaining run-time. Pure read of existing
 * CordisProject properties.
 *
 ...
 * `hop-on*` class names are unchanged so the existing styles apply.
 */
export default function HopOnHosts() {
  const [filters, setFilters] = useState({ programme: "", field: "", missingCountry: "", maxAgeMonths: 12 });
  const { loading, data } = useHopOnHosts(filters, true);
  ...
  return (
    <div className="hop-on dash-tool-panel__tool">
